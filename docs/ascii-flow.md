...
[Application Business Rules: UseCase]
    |  (Business logic: Create Pokémon, Replace types, Replace evolutions, Send mail)
    |  |  (Call abstract Repository to save Entity/BO data)
    |  |  (Call abstract Mail Adapter to send mail)
    ...
    |  |
    |  +--> [Interface Adapters: Mail Adapter] <-> [Frameworks & Drivers: Mail Service]
    |          (Send mail)
    v
[Interface Adapters: Controller]
